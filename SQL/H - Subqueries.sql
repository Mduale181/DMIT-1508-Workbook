...
SELECT StudentID
FROM   Activity
WHERE  PaymentTypeID = (SELECT PaymentTypeID
     FROM   PaymentType
     WHERE  PaymentTypeDescription = 'cash')
...
FROM   Student
WHERE  StudentID IN
	(SELECT DISTINCT ClubID FROM Club WHERE ClubName = 'Association of Computing Machinery')
...
FROM   Staff
WHERE  StaffID IN -- I used IN because the subquery returns many rows
	(SELECT StaffID FROM Registration WHERE CourseId = ( SELECT CourseId FROM Course WHERE CourseId= 'DMIT101' ))
